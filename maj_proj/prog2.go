...
func historyHandler(w http.ResponseWriter, r *http.Request) {
	...
	query := `from(bucket: "metrics0_1")
		|> range(start: -1h) // Récupère l'historique de la dernière heure
	...
	if err != nil {
		http.Error(w, "Erreur lors de la requête InfluxDB", http.StatusInternalServerError)
		return
	}
	...
	for result.Next() {
		if result.Err() != nil {
			http.Error(w, "Erreur lors de l'analyse des résultats", http.StatusInternalServerError)
			return
		}
